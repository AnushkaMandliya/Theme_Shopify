...
//custom cart drawer part 

document.addEventListener('cart:updated', syncDecimalCart);
document.addEventListener('DOMContentLoaded', syncDecimalCart);

function syncDecimalCart() {
  document.querySelectorAll('.decimal_qty_cart').forEach(wrapper => {

    const step = parseFloat(wrapper.dataset.step);
    const min  = parseFloat(wrapper.dataset.min);

    const decimalInput = wrapper.querySelector('.decimal_cart');
    const hiddenQty    = wrapper
      .closest('.cart-item__quantity-wrapper')
      .querySelector('.js-qty__num_custom_Cart');

    if (!decimalInput || !hiddenQty) return;

    // 🔥 integer → decimal conversion
    let decimalVal = hiddenQty.value * step;
    decimalInput.value = decimalVal.toFixed(2);
  });
}
